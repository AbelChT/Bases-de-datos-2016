...
CREATE VIEW ACTORES_DIRECTORES_TRIGGER AS
	SELECT director
	FROM DIRECTOR_Y_PELICULA
	INNER JOIN  ACT_DIRECTORES ON actor_director = director AND pelicula = director_y_pelicula.id;																			
-- Como resultado se muestra en esta view el nombre del actor/director																			


SELECT DISTINCT NOMBRE
	FROM PERSONA
	INNER JOIN ACTORES_DIRECTORES_TRIGGER ON PERSONA.ID = ACTORES_DIRECTORES_TRIGGER.director;
